...
\begin{figure}[htbp]
    \centering
    % Question 1
    % \subbottom[Comparison for Question 1]{%
    %   \begin{minipage}[b]{\textwidth}
    %     \centering
    %     \subbottom[Ground Truth\label{fig:q1_gt}]{%
    %       \includegraphics[width=0.3\linewidth]{q1_gt_map.html.pdf}}%
    %     \hfill
    %     \subbottom[Generated Map 1\label{fig:q1_pred1}]{%
    %       \includegraphics[width=0.3\linewidth]{q1_pred_map_1.html.pdf}}%
    %     \hfill
    %     \subbottom[Generated Map 2\label{fig:q1_pred2}]{%
    %       \includegraphics[width=0.3\linewidth]{q1_pred_map_2.html.pdf}}%
    %   \end{minipage}
    % }
    %\vspace{1em}
  
    % Question 7
    \subbottom[Comparison for Question 7: Which Freguesias have the highest male-to-female ratio in the population?]{%
      \begin{minipage}[b]{\textwidth}
        \centering
        \subbottom[Ground Truth\label{fig:q7_gt}]{%
          \includegraphics[width=0.3\linewidth]{q7_gt_map.html.pdf}}%
        \hfill
        \subbottom[Generated Map 1\label{fig:q7_pred1}]{%
          \includegraphics[width=0.3\linewidth]{q7_pred_map_1.html.pdf}}%
        \hfill
        \subbottom[Generated Map 2\label{fig:q7_pred3}]{%
          \includegraphics[width=0.3\linewidth]{q7_pred_map_3.html.pdf}}%
      \end{minipage}
    }
    %\vspace{1em}
  
    % Question 10
    \subbottom[Comparison for Question 10: In which Freguesias is the unemployment rate highest?]{%
      \begin{minipage}[b]{\textwidth}
        \centering
        \subbottom[Ground Truth\label{fig:q10_gt}]{%
          \includegraphics[width=0.3\linewidth]{q10_gt_map.html.pdf}}%
        \hfill
        \subbottom[Generated Map 1\label{fig:q10_pred1}]{%
          \includegraphics[width=0.3\linewidth]{q10_pred_map_1.html.pdf}}%
        \hfill
        \subbottom[Generated Map 2\label{fig:q10_pred3}]{%
          \includegraphics[width=0.3\linewidth]{q10_pred_map_3.html.pdf}}%
      \end{minipage}
    }
...
%The map generated for Q1 aims to display the percentage of buildings constructed before 1945 in each Freguesia. Despite the lower performance metrics, the visual output appears to reflect the expected geographical distribution of older buildings. However, upon closer examination, inconsistencies become evident. The percentages in some regions may not align with known historical data, suggesting potential errors in the calculation within the SQL query. Additionally, the color scale used may not appropriately represent the range of percentages, potentially misleading the interpretation of the data. Certain Freguesias may be missing from the map or display null values, indicating issues with data retrieval or handling of null values in the SQL query.

%These issues highlight the impact of errors in the SQL generation on the final visual output, emphasizing the importance of accuracy in earlier stages of the data processing pipeline.

\subsection{Analysis of Q10 Map}

%For Q10, the map displays the unemployment rate across Freguesias. Similar to Q1, the map provides a general overview but suffers from potential inaccuracies. The unemployment rate calculation involves dividing the sum of unemployed individuals by the number of economically active individuals, with potential division by zero. Errors in handling these cases can lead to incorrect rates. The map may show anomalously high or low unemployment rates in certain areas, which could result from incorrect data aggregation or misinterpretation of the SQL query. Moreover, the map's visual elements, such as the legend, color gradients, and labeling, may not effectively convey the information, making it difficult to interpret the results accurately.